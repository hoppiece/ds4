...
.PHONY: all help clean test test-metal-session-batch test-metal-io-smoke test-mxfp4-cuda test-cuda-session-batch test-cuda-mixed-batch dspark-acceptance dspark-verify-depth mtp-verify-depth cpu cuda cuda-spark cuda-generic cuda-regression strix-halo rocm
...
help:
	...
	@echo "  make test-metal-io-smoke  Build and run the standalone Metal IO smoke test"
...
tests/test_metal_io_smoke: tests/test_metal_io_smoke.m
	$(CC) $(OBJCFLAGS) -o $@ $< $(METAL_LDLIBS)

test-metal-io-smoke: tests/test_metal_io_smoke
	./tests/test_metal_io_smoke
...
help:
	...
	@echo "  make cpu                 Build CPU-only ./ds4, ./ds4-server, ./ds4-bench, ./ds4-eval, and ./ds4-agent"
	@echo "  make test                Build and run tests"
	@echo "  make test-metal-io-smoke Metal IO smoke test requires macOS 13 or newer"
	@echo "  make dspark-verify-depth Run DSpark speculative verification smoke if support GGUF is present"
	@echo "  make mtp-verify-depth    Run legacy MTP speculative verification smoke if MTP GGUF is present"
	@echo "  make clean               Remove build outputs"
...
test-metal-io-smoke:
	@echo "test-metal-io-smoke requires macOS 13 or newer"
